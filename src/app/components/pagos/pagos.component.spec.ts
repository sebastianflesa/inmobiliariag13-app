...
beforeEach(async () => {
  ...
  pagoServiceSpy.getPagos.and.returnValue(of({ data: [] }));
  ...
  spyOn(PagosComponent.prototype, 'ngOnInit');
...
it('should load contratos', () => {
  contratoServiceSpy.getContratos.and.returnValue(of({ data: [{ id: 1 }] }));
  ...
  it('should load pagos', () => {
    pagoServiceSpy.getPagos.and.returnValue(of({ data: [{ id: 100 }] }));
    component.loadPagos();
    expect(pagoServiceSpy.getPagos).toHaveBeenCalledTimes(1);
  ...
  it('should create pago', () => {
    ...
    component.crearPago();
    expect(pagoServiceSpy.createPago).toHaveBeenCalledTimes(1);
  });

  it('should handle error creating pago', () => {
    pagoServiceSpy.createPago.and.returnValue(
      throwError(() => ({ error: { message: 'Falló' } }))
    );
